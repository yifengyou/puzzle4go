...
func Producer(data []uint64) {
	for {
		time.Sleep(1 * time.Second)
		cargo.Lock.Lock()
		cargo.Data = append(cargo.Data, data...)
		cargo.Lock.Unlock()
		fmt.Println("producer append", data)
	}
}
...
func Consumer() error {
	for {
		...
		if len(cargo.Data) > 0 {
			ret := cargo.Data[0]
			fmt.Println("comsumer consu", ret, " len:", len(cargo.Data))
			cargo.Data = cargo.Data[1:]
			cargo.Lock.Unlock()

		} else {
			cargo.Lock.Unlock()
			fmt.Println("no data for consumer")
			time.Sleep(1 * time.Second)
		}
	}
}
...
func TestCargo(t *testing.T) {
	cargo.Data = make([]uint64, 100)
	for i := 1; i < 10; i++ {
		randomData := rand.Uint64()
		go Producer([]uint64{randomData})
